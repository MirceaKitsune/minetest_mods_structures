...
local function mapgen_buildings_draw (pos, size, angle, floors, entry)
	...
	if floors == 0 then
		...
		table.insert(new_scheme, {name = entry.name, pos = pos, angle = angle, size = size, base = entry.base, replacements = entry.replacements, force = entry.force, chaining = nil})
	else
		...
		table.insert(new_scheme, {name = entry.name_start, pos = pos, angle = angle, size = size_start, base = entry.base, replacements = entry.replacements, force = entry.force, chaining = nil})
		...
		for height = height_middle_start, height_middle_end, size_middle.y do
			...
			table.insert(new_scheme, {name = entry.name, pos = pos_middle, angle = angle, size = size_middle, base = nil, replacements = entry.replacements, force = entry.force, chaining = nil})
		end
		...
		table.insert(new_scheme, {name = entry.name_end, pos = pos_end, angle = angle, size = size_end, base = nil, replacements = entry.replacements, force = entry.force, chaining = nil})
	end
