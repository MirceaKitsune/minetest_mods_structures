...
local function group_size (id)
	...
	for i, entry in ipairs(structures.mapgen_table[id].buildings) do
	...
	-- loop through roads
	for i, entry in ipairs(structures.mapgen_table[id].roads) do
		local size = io_get_size(0, entry.name or entry.name_I)
...
local function mapgen_generate (minp, maxp, seed)
	...
	if not structures.mapgen_cubes[cube_index].structures then
		...
		local groups_id = {}
		for i, entry in pairs(structures.mapgen_table) do
			...
			if height_min >= structures.mapgen_cubes[cube_index].minp.y and height_max <= structures.mapgen_cubes[cube_index].maxp.y then
		...
		if #groups_id > 0 then
			...
			local group = structures.mapgen_table[group_id]
			...
			if group.probability >= math.random() then
				...
				local position_start = {
					x = math.random(structures.mapgen_cubes[cube_index].minp.x, structures.mapgen_cubes[cube_index].maxp.x - group.size_horizontal + 1),
					y = structures.mapgen_cubes[cube_index].minp.y,
					z = math.random(structures.mapgen_cubes[cube_index].minp.z, structures.mapgen_cubes[cube_index].maxp.z - group.size_horizontal + 1),
				}
				local position_end = {
				...
				for w, road in ipairs(schemes_roads) do
					table.insert(structures.mapgen_cubes[cube_index].structures, road)
				end
			end
		end
	end
	...
	if structures.mapgen_cubes[cube_index].structures then
		local group_id = structures.mapgen_cubes[cube_index].group
		local group = structures.mapgen_table[group_id]
		-- schematics: name [1], position [2], angle [3], size [4]
		for i, structure in pairs(structures.mapgen_cubes[cube_index].structures) do
...
function structures:define(def)
	table.insert(structures.mapgen_table, def)

	-- calculate the size of this group, and store it as a set of extra properties
	local size_horizontal, size_vertical = group_size(#structures.mapgen_table)
	structures.mapgen_table[#structures.mapgen_table].size_horizontal = size_horizontal
	structures.mapgen_table[#structures.mapgen_table].size_vertical = size_vertical
	...
	local largest_vertical = (structures.mapgen_table[#structures.mapgen_table].noiseparams.scale + size_vertical) * structures.mapgen_cube_multiply_vertical
